...
# This script differs from und-activate.sh to register Floating License Server for understand 5 in legacy mode.
...
if [ ! -z "${UNDERSTAND_FLOATING_LICENSE_SERVER}" ]; then
    ...
    echo "Server: ${UNDERSTAND_FLOATING_LICENSE_SERVER} 00000000 ${UNDERSTAND_FLOATING_LICENSE_SERVER_PORT}" > "${UNDERSTAND_HOME}/conf/license/locallicense.dat"
    echo "[license]
legacylicensing=true   " > ~/.config/SciTools/License.conf
fi
